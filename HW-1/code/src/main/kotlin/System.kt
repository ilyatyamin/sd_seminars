...
class System(private var inputer: IInputer) : ICinemable, IEdditable, IMarkable {
    ...
    init {
        ...
        if (currentFilms.isNotEmpty()) {
            filmId = currentFilms.maxOf { it.id }
        }
    ...
    fun isCorrectUser(login: String, passw: String): User? {
        val user = users.find { it.tryToEnter(login, passw) }
        if (user == null) {
            return null
        } else {
            isEntered = true
    ...
    fun addNewUser(login: String, passw: String) {
        users.add(User(login, passw))
        serializeAll()
    ...
    override fun addFilm() {
        // Используя ConsoleInputer, считываем данные о фильме с консоли
        currentFilms.add(inputer.readFilmData())
        serializeAll()
    ...
    override fun addSession() {
        // Используя ConsoleInputer, считываем данные о сеансе с консоли
        val session = inputer.readSessionData(currentFilms)
        if (session != null) {
    ...
    override fun buyTicket() {
        // Используя ConsoleInputer, считываем данные о билете с консоли
        val ticket = inputer.readInfoForTicket(activeSessions)
        if (ticket != null) {
    ...
    override fun returnTicket() {
        // Используя ConsoleInputer, считываем данные о билете с консоли
        val ticket = inputer.readInfoForReturn(tickets)
        if (ticket != null) {
            // Находим сеанс в списке сеансов
            val session = activeSessions.find { it.id == ticket.session }
    ...
    override fun enterHallParameters() {
        val params = inputer.readHallParameters()
        pair = params
    ...
    override fun editFilm() {
        if (currentFilms.isEmpty()) {
            ...
            val film = inputer.readFilmId(currentFilms)
            if (film != null) {
                println("Что вы хотите поменять в фильме? Название (1), режиссер (2), длительность (3) или возрастное ограничение (4)? ")
                ...
                when (readln().toIntOrNull()) {
                    ...
                    2 -> {
                        ...
                        film.director = readln()
                    }

                    3 -> {
                        // Изменение продолжительности фильма
                        film.duration = inputer.readNonZeroLong().minutes
                    }

                    4 -> {
                        // Изменение возрастного ограничения фильма
                        film.ageLimits = inputer.readAgeLimit()
                    }
    ...
    override fun editSession() {
        ...
        if (activeSessions.isEmpty()) {
            ...
            val session = inputer.readSessionId(activeSessions)
            if (session != null) {
    ...
    override fun comeToSession() {
        if (activeSessions.isEmpty()) {
            ...
            if (ticket != null) {
                var session = activeSessions.find{it.id == ticket.session}
    ...
    private fun readDataFromFile() {
        ...
        try {
            currentFilms = Json.decodeFromString(writer.read(pathToFilms)!!)
        } catch (_: Exception) {
        }
        try {
    ...
    private fun serializeAll() {
        writer.write(Json.encodeToString(currentFilms), pathToFilms)
