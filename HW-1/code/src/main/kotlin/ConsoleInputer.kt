...
class ConsoleInputer : IInputer {
     ...
     * */
    override fun readFilmData(): Film {
        ...
        return Film(tempName, duration, tempProd, tempAge)
    }

    /** Считывает с консоли информацию о сеансе
     * @return объект типа Session или null, в случае возникновения ошибки */
    override fun readSessionData(listFilms: MutableList<Film>): Session? {
        ...
        val film: Film = listFilms.find { it.id == id }!!
        ...
        while (dateTime == null) {
            try {
                ...
                // Проверяем, что человек не хочет создать сеанс в прошлом
        ...
        return Session(film, dateTime.format(DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm")), Hall(pair.first, pair.second))
    }
    ...
    override fun readFilmId(listFilms: MutableList<Film>): Film? {
        ...
        return listFilms.find { it.id == id }
    }
